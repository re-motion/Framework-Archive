...
public interface IFormGridRowProvider: IControl
{
  ...
  /// </summary>
  /// <param name="table"> The UniqueID of the <see cref="HtmlTable"/> whose rows will be hidden. </param>
  /// <returns> A <see cref="StringCollection"/> containing the IDs. </returns>
  StringCollection GetHiddenRows (string tableUniqueID);

  /// <summary>
  ///   Returns a list of <see cref="FormGridRowInfo"/> objects used to constrtuct and then 
  ///   insert new rows into a form grid.
  /// </summary>
  /// <param name="table"> The UniqueID of the <see cref="HtmlTable"/> into which the new rows will be inserted. </param>
  /// <returns> A <see cref="FormGridRowInfoCollection"/> containing the prototypes. </returns>
  FormGridRowInfoCollection GetAdditionalRows (string tableUniqueID);
}
