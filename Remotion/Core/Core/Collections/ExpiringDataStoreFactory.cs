...
namespace Remotion.Collections
{
  ...
  {
    /// <summary>
    /// Creates a <see cref="ExpiringDataStore{TKey,TValue, TExpirationInfo}"/> instance that is not thread-safe and uses the specified
    /// <see cref="IEqualityComparer{T}"/>.
    /// </summary>
    /// <typeparam name="TKey">The type of the keys.</typeparam>
    /// <typeparam name="TValue">The type of the values.</typeparam>
    /// <typeparam name="TExpirationInfo">The type of the expiration info.</typeparam>
    ...
    /// <returns>
    /// A <see cref="ExpiringDataStore{TKey,TValue,TExpirationInfo}"/> instances for storing keys and values.
    /// </returns>
    public static ExpiringDataStore<TKey, TValue, TExpirationInfo> Create<TKey, TValue, TExpirationInfo> (
        IExpirationPolicy<TValue, TExpirationInfo> policy, IEqualityComparer<TKey> comparer)
    {
      return new ExpiringDataStore<TKey, TValue, TExpirationInfo> (policy, comparer);
    }
    ...
    /// complete. When the factory delegates take a long time to execute, consider using <see cref="CreateWithLazyLocking{TKey,TValue,TExpirationInfo}"/> 
    /// instead to reduce contention.
    /// </remarks>
    public static LockingDataStoreDecorator<TKey, TValue> CreateWithLocking<TKey, TValue, TExpirationInfo> (
        IExpirationPolicy<TValue, TExpirationInfo> policy, IEqualityComparer<TKey> comparer)
    {
      return new LockingDataStoreDecorator<TKey, TValue> (new ExpiringDataStore<TKey, TValue, TExpirationInfo> (policy, comparer));
    }
    ...
    /// delegates do not take a long time, consider using <see cref="CreateWithLocking{TKey,TValue,TExpirationInfo}"/> instead to reduce the number of locks used.
    /// </remarks>
    public static LazyLockingDataStoreAdapter<TKey, TValue> CreateWithLazyLocking<TKey, TValue, TExpirationInfo> (
        IExpirationPolicy<DoubleCheckedLockingContainer<TValue>, TExpirationInfo> policy, IEqualityComparer<TKey> comparer) where TValue: class
    {
      return
          new LazyLockingDataStoreAdapter<TKey, TValue> (
              new ExpiringDataStore<TKey, DoubleCheckedLockingContainer<TValue>, TExpirationInfo> (policy, comparer));
    }
  }
}