...
{
  /// <summary>
  /// Implements the <see cref="IDocument"/> interface as a mixin agnostic of its specific target type. Requires the target type to (directly
  ...
  /// of the target object in order to implement business logic to be executed before the commit.
  /// </summary>
  public class DocumentMixin : DomainObjectMixin<ITenantBoundObject>, IDocument
  {
    ...
    private void Target_Committing (object sender, EventArgs e)
    {
      if (string.IsNullOrEmpty (Title))
        Title = "(unnamed document of " + This.Tenant + ")";
    }
  }
}