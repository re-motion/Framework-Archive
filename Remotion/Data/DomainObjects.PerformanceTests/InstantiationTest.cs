...
namespace Remotion.Data.DomainObjects.PerformanceTests
{
  ...
  public class InstantiationTest : DatabaseTest
  {
    ...
    public void GetObjectReference ()
    {
      ...
      LifetimeService.GetObjectReference (transaction, ObjectID.Create(typeof (Person), Guid.NewGuid ()));

      stopwatch.Start ();
      for (int i = 0; i < TestRepititions; i++)
        found &= LifetimeService.GetObjectReference (transaction, ObjectID.Create(typeof (Person), Guid.NewGuid())) != null;
      stopwatch.Stop ();
