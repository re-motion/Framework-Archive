...
namespace Remotion.Data.UnitTests.DomainObjects.Core.DataManagement.Commands
{
  ...
  public class UnloadAllCommandTest : StandardMappingTest
  {
    ...
    {
      var dataContainer = DataContainer.CreateForExisting (ObjectID.Create(typeof (Order), Guid.NewGuid ()), null, pd => pd.DefaultValue);
    ...
    {
      var dataContainer = DataContainer.CreateNew (ObjectID.Create(typeof (Order), Guid.NewGuid ()));
      dataContainer.SetDomainObject (DomainObjectMother.CreateFakeObject<Order> (dataContainer.ID));
      return dataContainer;
    }
  }
