...
{
  // TODO Review 2982: Fix the tests below by adding new rows to the database. To make this easier, you can write a test that builds the desired 
  // TODO Review 2982: data structure (creating or loading objects via NewObject()/GetObject()), then commits that transaction using 
  // TODO Review 2982: SetDatabaseModifyable()/ClientTransaction.Current.Commit(). Fire up SQL profiler and log the INSERT/UPDATE statements, then 
  // TODO Review 2982: add those statements to DataDomainObjects_CreateTestData.sql. Also document the data structure in the DomainObjectIDs class. 
  // TODO Review 2982: Note that you have to "Clean" the project before building if you make changes only to the SQL script; otherwise your tests
  // TODO Review 2982: will still run with the old SQL script...
  [TestFixture]
  public class MixinAddedPropertyIntegrationTest : IntegrationTestBase
  {
    ...
    [Test]
    [Ignore ("TODO Review 2982: See above.)")]
    ...
    [Test]
    [Ignore ("TODO Review 2982: This test does not work because DefaultMappingResolutionStage.ResolveCollectionSourceExpression cannot handle "
    ...
    public void CollectionValuedRelationAddedByMixin_UsedInFromExpression ()
    {
      var query =
          ...
          select related;
      // TODO Review 2982: To get a more meaningful result, also add a few data records for RelationTargetForPersistentMixin that point back to some TargetClassForPersistentMixins via RelationProperty3
