...
{
  [TestFixture]
  public class CommitTest : InactiveTransactionsTestBase
  {
    ...
    [Test]
    public void CommitInInactiveRootTransaction_IsForbidden ()
    {
      ...
      CheckForbidden (() => InactiveRootTransaction.Commit (), "TransactionCommitting");
    ...
    [Test]
    public void CommitInInactiveMiddleTransaction_IsForbidden ()
    {
      Assert.That (InactiveRootTransaction.HasChanged (), Is.True);
      Assert.That (InactiveMiddleTransaction.HasChanged (), Is.True);
      Assert.That (ActiveSubTransaction.HasChanged (), Is.True);

      CheckForbidden (() => InactiveMiddleTransaction.Commit (), "TransactionCommitting");
