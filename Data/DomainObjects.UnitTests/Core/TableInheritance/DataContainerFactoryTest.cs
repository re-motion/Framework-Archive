    ...
    public void RelationClassIDColumnRefersToAbstractClass ()
    {
      ...
      using (IDbCommand command = builder.Create ())
      {
        using (IDataReader reader = command.ExecuteReader ())
        {
          ...
          {
            string expectedBeginOfMessage = string.Format ("Error while reading property 'Remotion.Data.DomainObjects.UnitTests.Core.TableInheritance.TestDomain.Order.Customer' of object '{0}':", id);
